...
class ReportBuilder:
    ...
    def _format_levels_for_timeframe(self, supports: List[Level], resistances: List[Level], tf: str) -> Dict[str, str]:
        """Formats levels into a dictionary by matching exact names from analysis modules."""
        level_data = {}

        # This map now matches the exact names generated by the updated analysis modules.
        # The keys here correspond to the placeholders in the templates.
        trend_name_map = {'1d': 'طويل المدى', '4h': 'متوسط المدى'}
        trend_name = trend_name_map.get(tf, 'قصير المدى')

        level_map = {
            f'support_trend_{tf}': (f"دعم الاتجاه {trend_name}", supports),
        ...
        # Add pattern target separately, as it has a generic name
        for r in resistances:
            if "pattern target" in r.name.lower():
                level_data[f'pattern_target_{tf}'] = f"${r.value:,.2f}"
                break

        # Iterate through the map and find the first level with an exact name match
        for key, (exact_name, levels) in level_map.items():
            found_level = "N/A"
            for level in levels:
                if level.name == exact_name:
