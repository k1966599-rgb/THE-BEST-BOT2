...
class InteractiveTelegramBot(BaseNotifier):
    def __init__(self, config: Dict, fetcher: BaseDataFetcher, orchestrator: AnalysisOrchestrator, decision_engine: DecisionEngine):
        ...
        self.followed_trades: Dict[str, TradeSetup] = {}
    ...
    async def _run_analysis_for_request(self, symbol: str, timeframes: list, analysis_type: str, chat_id: int) -> Dict[str, Any]:
        ...
        for tf in timeframes:
            try:
                ...
                if recommendation.get('trade_setup'):
                    recommendation['trade_setup'].chat_id = chat_id
    ...
    async def _main_button_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if callback_data == "start_menu":
            ...
            try:
                report_parts = await self._run_analysis_for_request(symbol, timeframes, analysis_name, query.message.chat_id)
    ...
    async def _send_trade_alert(self, trade_setup: TradeSetup, alert_type: str):
        """
        ...
        await self.bot.send_message(chat_id=trade_setup.chat_id, text=message, parse_mode='HTML')
