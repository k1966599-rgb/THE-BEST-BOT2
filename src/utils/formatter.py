...
def format_analysis_from_template(analysis_data: Dict[str, Any], symbol: str, timeframe: str, lang: str = "ar") -> str:
    ...
    if final_reason and isinstance(final_reason, dict):
        reason_key = final_reason.get('key', 'N/A')
    ...
    if volume_sma > 0 and current_volume > 0:
        if current_volume > volume_sma * 1.5: volume_analysis_text = "حجم تداول مرتفع"
        elif current_volume < volume_sma * 0.5: volume_analysis_text = "حجم تداول منخفض"
    ...
    replacements = {
        ...
        "candlestick_pattern": analysis_data.get('pattern', 'N/A'),
